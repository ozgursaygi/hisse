...
# ============================================================
# BUY_ACTIONS / SELL_ACTIONS list which tiers a board accepts, ordered
# STRONGEST FIRST — that order also drives the badge and the sorting.
#   ('STRONG BUY', 'BUY')  -> both tiers          (default)
#   ('STRONG BUY',)        -> v24/v25.0 behaviour (strong only)
# REQUIRE controls how the three signals must line up:
#   'any'   -> any one of Technical / Model / Forecast hits the tier
#   'all'   -> all three must hit it
#   'model' -> only the NOW·Model signal counts
# MAX = 0 -> show everything; >0 -> cap the list.
...
def _board_html(results, actions, css_class: str, title_word: str,
  ...
  <section class="{css_class}">
    <div class="lb-head">
      ...
      <div class="sub">Da&#287;&#305;l&#305;m: <b>{breakdown}</b> &middot; ko&#351;ul: <b>{require}</b> &middot;
