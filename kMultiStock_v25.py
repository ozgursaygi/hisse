...
def _board_html(results, action: str, css_class: str, title_word: str,
                require: str, max_n: int, reverse: bool) -> str:
    rows = [r for r in results
            if r is not None and r.get('res') is not None and r['res'].ok
            and r.get('sig') and _sig_is_action(r['sig'], action, require)]

    if not rows:
        return f"""
  <section class="{css_class}">
    <div class="lb-head">
      <div class="kicker">Signals &middot; {action}</div>
      <h2>{action} Sinyali Olan Hisse Yok</h2>
      <div class="sub">Bu ko&#351;ulda ('{require}') hi&ccedil;bir hisse {action} vermedi.</div>
    </div>
  </section>"""

    rows.sort(key=lambda r: float(r['sig'].get('now_pred', 0.0)), reverse=reverse)
  ...
  <section class="{css_class}">
    <div class="lb-head">
      <div class="kicker">Signals &middot; {action}</div>
      <h2>{action} Sinyali Olan {len(rows)} Hisse</h2>
      <div class="sub">Ko&#351;ul: <b>{require}</b> &middot; en g&uuml;&ccedil;l&uuml; {title_word} &uuml;stte.
        Sinyaller ham g&ouml;sterilir; g&uuml;venilirlik notu her hissenin kendi kart&#305;ndad&#305;r.{rel_note}
...
def build_strong_buy_html(results, require: str = STRONGBUY_REQUIRE,
    ...
    return _board_html(results, 'STRONG BUY', 'strongbuy', 'model &ccedil;a&#287;r&#305;s&#305;',
                       require, max_n, reverse=True)


def build_strong_sell_html(results, require: str = STRONGSELL_REQUIRE,
                           max_n: int = STRONGSELL_MAX) -> str:
    return _board_html(results, 'STRONG SELL', 'strongsell', 'sat&#305;&#351; &ccedil;a&#287;r&#305;s&#305;',
