...
#   [FIX-8] Tahmin ufku 7 güne indirildi; ana grafiğe test dönemi çizgisi eklendi.
# ============================================================
...
PREDICTION_HORIZON = 7  # 7 günlük tahmin (daha kısa ufuk)
...
def get_stock_data(symbol, macro_df, news_score=0.0):
    ...
    try:
        ...
        # N günlük gelecek getiri (direkt hedef)
        df['Future_Ret_Nd'] = np.log(df['Close'].shift(-PREDICTION_HORIZON) / df['Close'])
        ...
        if not macro_df.empty:
            ...
            df = df.dropna()
        df = df.dropna(subset=['Future_Ret_Nd'])
        return df
    except Exception as e:
        print(f"HATA: İndikatörler hesaplanamadı: {e}")
        return None

# --- YILLIKLANDIRILMIŞ SHARPE HESAPLAMA (PERİYODA GÖRE) ---
def annualized_sharpe(period_returns, periods_per_year, risk_free_rate=0.045):
    """Dönem getirileri için yıllıklandırılmış Sharpe oranı"""
...
def calculate_real_metrics(returns_array, periods_per_year):
...
# --- BACKTEST (N GÜNLÜK SİNYAL) ---
def backtest_Nd(actual_returns, predicted_returns):
    """N günlük tahminlere göre strateji getirilerini hesaplar"""
...
def plot_main_chart(df, val_split, test_dates, rec_prices, fut_dates, target_price, ci_low, ci_high, sig_color, name, ticker):
    fig, ax = plt.subplots(figsize=(14, 7))
    ax.plot(df.index[-200:], df['Close'].iloc[-200:], label="Gerçek Fiyat", color="#1f2937", linewidth=1.8)
    # Test dönemi tahmin çizgisi
    if test_dates is not None and rec_prices is not None and len(test_dates) > 0:
        ax.plot(test_dates, rec_prices,
                label=f"Model Test (OOS, {PREDICTION_HORIZON}g)", linestyle="--", color="#f59e0b", linewidth=1.5, alpha=0.8)
...
def plot_scatter_predictions(actual, predicted, ticker):
    """Gerçekleşen vs Tahmin Edilen N günlük getiri saçılım grafiği"""
    ...
    ax.set_xlabel(f'Gerçekleşen {PREDICTION_HORIZON} Günlük Getiri'); ax.set_ylabel(f'Tahmin Edilen {PREDICTION_HORIZON} Günlük Getiri')
...
<strong>v10.4 İyileştirmeleri:</strong> Eksiksiz veri sızıntısı kontrolü | Doğru DM testi yorumu | Düzeltilmiş Sharpe yıllıklandırması | Dinamik sinyal eşiği | Gerçekçi performans grafikleri | Tahmin ufku 7 gün
</div>
"""
...
{science_html}
<div class="note">
<b>R² yorum notu:</b> Burada R², {PREDICTION_HORIZON} günlük log-getiri üzerinde hesaplanır. Finansal getiri serilerinde
...
<div class="stat-box"><div class="stat-label">Hedef Fiyat ({PREDICTION_HORIZON}G)</div><div class="stat-val">{currency}{m['target_price']:.2f}</div><div class="stat-sub">Doğrudan {PREDICTION_HORIZON}g Getiri</div></div>
...
<div class="stat-box"><div class="stat-label">Strateji Sharpe</div><div class="stat-val">{m['bt_sharpe']:.2f}</div><div class="stat-sub">Backtest ({PREDICTION_HORIZON}g, düzeltilmiş)</div></div>
...
def analyze():
    set_seeds()
    periods_per_year = 252 / PREDICTION_HORIZON  # Yıllıklandırma faktörü
    print("="*65)
    ...
    # Tarihsel N-gün getiri oynaklığı (sinyal eşiği için)
    hist_vol_Nd = np.std(df['Future_Ret_Nd'].dropna().tail(252)) # son 1 yıl
    print(f"   Tarihsel {PREDICTION_HORIZON}g getiri oynaklığı: {hist_vol_Nd:.5f}")

    exclude_cols = ['Open', 'High', 'Low', 'Volume', 'Close', 'Adj Close']
    features = ['Close'] + [c for c in df.columns if c not in exclude_cols]
    target_idx = features.index('Future_Ret_Nd')
    data = df[features].values
    print(f"   Feature sayısı: {len(features)} (Hedef: {PREDICTION_HORIZON} günlük getiri)")
    ...
    print(f"\n5. Walk-Forward Cross-Validation ({PREDICTION_HORIZON}g hedef):")
    ...
    print(f"\n7. Test seti değerlendirmesi (Out-of-Sample, {PREDICTION_HORIZON}g getiri):")
    ...
    # Test dönemi için fiyat tahminleri (grafikte göstermek için)
    test_close = df['Close'].iloc[val_split:].values
    rec_prices = test_close * np.exp(pred_returns)  # tahmin edilen N gün sonu fiyat
    actual_future_close = df['Close'].iloc[val_split:].shift(-PREDICTION_HORIZON).dropna()
    rec_prices = rec_prices[:len(actual_future_close)]
    test_dates = df.index[val_split:][:len(rec_prices)]

    # DM testi ve doğru yorum
    train_Nd_ret = df['Future_Ret_Nd'].iloc[:val_split].dropna().values
    pred_naive = baseline_naive(actual_returns)
    pred_rw = baseline_random_walk(train_Nd_ret, actual_returns)
    ...
    print(f"\n8. Backtest ({PREDICTION_HORIZON}g sinyaller):")
    strategy_rets = backtest_Nd(actual_returns, pred_returns)
    bt_sharpe = annualized_sharpe(strategy_rets, periods_per_year)
    bh_sharpe = annualized_sharpe(actual_returns, periods_per_year)
    ...
    if abs(predicted_ret) > 0.5 * hist_vol_Nd:
        if predicted_ret > 0:
            signal, sig_color, sig_desc = ("GÜÇLÜ AL", "green", "Pozitif Trend") if abs(predicted_ret) > 1.5*hist_vol_Nd else ("AL / TUT", "blue", "Zayıf Yükseliş")
        else:
            signal, sig_color, sig_desc = ("SAT", "red", "Negatif Trend") if abs(predicted_ret) > 1.5*hist_vol_Nd else ("ZAYIF SAT", "orange", "Zayıf Düşüş")

    fut_dates = pd.date_range(df.index[-1] + timedelta(days=1), periods=PREDICTION_HORIZON, freq='D')
    ...
    # Grafikler
    chart_b64 = plot_main_chart(df, val_split, test_dates, rec_prices, fut_dates, target_price, ci_low, ci_high, sig_color, TICKER_NAME, TICKER)
    ...
    extras = {
        ...
        'ma_cross': mini_plot(df[['SMA50', 'SMA200']].tail(180), None, 'SMA50/200'),
    ...
    <h3>Bilimsel Doğrulama Sonuçları ({PREDICTION_HORIZON}g Getiri Hedefi)</h3>
    <table>
